...
namespace HabitTracker
{
	public class MySqliteTools
	{
		...
		public static void CreateNewRecord(object[] newRecord) //newRecord should be { DateOnly, int }
		{
			using (var connection = new SqliteConnection(connectionString))
			{
				// This is the command to send to the DB
				...
				VALUES ('{newRecord[0].ToString()}', {newRecord[1]});";
		...
		public static void UpdateExistingRecord(int id, object[] newRecord)
		{
			using (SqliteConnection connection = new SqliteConnection(connectionString))
			{
				// This is the command to send to the DB
				using (SqliteCommand tableCmd = connection.CreateCommand())
				{
					...
					tableCmd.CommandText =
						$@"UPDATE CodingHours
					SET Date = '{newRecord[0].ToString()}', Quantity = {newRecord[1]}
					WHERE Id = {id}";
